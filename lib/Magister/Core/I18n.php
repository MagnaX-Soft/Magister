...
abstract class I18nSource {
	...
	*/
	 ...
	 * @return string
	*/
	public function singular($singular, $domain) {
		return $this->translate($singular, $domain);
	}
	 ...
	 * @return string
	 */
	public function plural($singular, $plural, $number, $domain) {
		return $this->translate(array($singular, $plural, $number), $domain);
	}
...
class GettextI18nSource extends I18nSource {
	...
	public function addTranslation($location, $domain) {
		...
		if (2500072158 == $input[1]) //"950412de"
			$bigEndian = false;
		elseif (3725722773 == $input[1]) //"de120495"
		$bigEndian = true;
		else {
			fclose($file);
			throw new I18nException('\'' . $location . '\' is not a gettext file');
		}
		// read revision - not supported for now (offset 4)
		$input = $this->readMOData($file, 1, $bigEndian);

		// number of strings (offset 8)
		$input = $this->readMOData($file, 1, $bigEndian);
		$total = $input[1];

		// offset of original strings (offset 12)
		$input = $this->readMOData($file, 1, $bigEndian);
		$OOffset = $input[1];

		// number of translation strings (offset 16)
		$input = $this->readMOData($file, 1, $bigEndian);
		$TOffset = $input[1];

		// The hashing table is defined at offsets 20 & 24,
		// but we aren't using it as we store the entries in
		// php arrays (we let php handle the hashing for us)
	...
	public function translate($messageId, $domain) {
		...
		!array_key_exists($messageId, $this->translate[$domain]))
			return $messageId;
        ...
        $rule = $this->getPlural($number, $plocale);
        do {
        	if (isset($this->translate[$domain][$messageId][$rule]))
        		return $this->translate[$domain][$messageId][$rule];
        } while (--$rule >= 0);
