...
class Restler {
    ...
    /**
     * Constructor
     * @param bool $production_mode When set to FALSE, it will run in
     ...
     * @param array class name of the format class that implements iFormat
     * @example $restler->setSupportedFormats('JsonFormat', 'XmlFormat'...);
     */
    public function setSupportedFormats(array $args) {
        $extensions = array();
     ...
     * @param string $class_name name of the service class
    ...
    public function saveCache() {
        ...
        foreach ($this->routes as $key => $value) {
            ...
            foreach ($value as $ke => $va) {
                ...
                $s .= '$o[\'' . $key . '\'][\'' . $ke . '\']=' . str_replace(PHP_EOL, PHP_EOL . "\t", var_export($va, TRUE)) . ';';
            }
        }
        $s .= PHP_EOL . 'return $o;';
    ...
    protected function mapUrlToMethod() {
        ...
        foreach ($urls as $url => $call) {
            ...
            } elseif (substr($this->url, -strlen($url)) == $url) {
                $found = TRUE;
                break;
            }
        }
...
interface iFormat {
     ...
     * @param bool $human_readable set to TRUE when restler
 ...
 * @param bool $utf_encode encode the strings as utf8
 * @category   Framework
 * @package    Restler
 * @subpackage Format
 * @author     R.Arul Kumaran <arul@luracast.com>
